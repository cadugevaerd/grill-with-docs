...
Use the git/bugfix/verify-review-ship extensions with the stated minimum version. For release zip artifacts, fingerprint inputs and verify the executable bit (`chmod +x`) plus an actual execution test. Fingerprint workflow, registry and constitution in state. Do not overwrite an incompatible human WORKFLOW. Projetos v2/v3/v4 permanecem byte-intactos. Não há migração automática para v5. Workflow humano equivalente só é aceito como v5 se declarar a fronteira e o registry v5. This file is generic and contains no project data.

## Eficiência e revisão v5

Use `assets/agent-orchestration.v2.json` e `references/agent-orchestration.v2.md`.
Revisões fixas em plan e review, com classificação por etapa e exceções obrigatórias.
Dois grupos de workers por padrão; uma sessão por nó, com resultados por tarefa.
Até três perguntas independentes por lote, uma transição por DQ respondida.
Os limites de entrevista contam perguntas materiais, não lotes.
