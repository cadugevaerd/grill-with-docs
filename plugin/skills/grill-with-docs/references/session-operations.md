# Operações de sessão e contratos históricos

Para v5, a matriz e entrevista do protocolo atual prevalecem sobre as regras v1 abaixo.
As demais garantias e argumentos permanecem válidos.
...
Para especialista Orca, faça a primeira chamada `accept` sem observação para gravar `--result` (`RESULT-RECORDED`) ou `--diagnostic` (`FAILED`); confirme `worker_done` aceito, execute `worker-release --dispatch`, confira `worker-show` do mesmo Dispatch e só então repita `accept` com a mesma referência e a observação fechada. O diagnóstico continua `FAILED`, mas seu recurso fecha com prova. Se settlement ou release falhar, conserve `CLOSE_PENDING` e recupere a mesma operação sem repetir o resultado.

Autor de todo COMO: Codex família `astra` (slug de menor `priority` em `$CODEX_HOME/models_cache.json`; catálogo ausente ou sem a família recusa `TIER-MODEL-UNRESOLVED`) `/xhigh`, Claude `opus/xhigh`; revisor de julgamento: o mesmo modelo obrigatório do runtime, `high`, em sessão/incarnation distinta de todos os autores dos inputs correntes. Passes mistos são separados. `--author-activity` correlaciona autoria; `--files` repete por arquivo e reviewer não recebe escrita. Checks determinísticos são reproduzíveis e não produzem revisão de julgamento. Implementadores conservam o tier não-frontier e os verbos do scheduler.
...
`gauntlet-context-takeover` exige prova terminal do líder anterior. Após essa prova, workers exatamente em `PREPARED` são herdados pelo contexto sucessor e aparecem em `inherited_prepared_workers`; o conjunto também integra o hash da prévia. Atividades em voo, workers em qualquer outro estado ativo e observações desconhecidas continuam recusando `TAKEOVER-WORK-ACTIVE`. Essa exceção é exclusiva do takeover: prepare-switch e resume continuam exigindo quiescência integral.

`gauntlet-activity-fence ROOT --work-id ID --activity-id ID --session-ref REF --authorization PATH` encerra sem aceite uma atividade de especialista, em prévia por padrão; a efetivação exige `--apply --expected-sha256 SHA`. Admite duas formas: `DISPATCHED` órfã e `RESULT_RECORDED` de sessão retida. Exige duas provas terminais obtidas do ambiente: o dispatch do especialista (por `orca:` mais `owner_dispatch`) e o dispatch do líder da atividade. Autoridade: com o líder vivo, só o líder corrente exato; com o líder terminal, o sucessor com a própria sessão provada por readiness. Exige autorização humana exata `human-authorization/v1` com escopo work item, contexto e atividade. Efeito: atividade `FAILED` com `diagnostic_ref`, recurso `CLOSED` com receipt correlacionado e operação `activity-fence` `CONFIRMED`. O resultado cercado nunca é aceito nem herdado; a reexecução nasce como atividade nova. `gauntlet-cleanup` não reconhece esse fechamento nesta versão e o reporta como `SESSION-CLOSE-UNPROVEN`, sem bloquear takeover, switch, phase-turn ou ship (DQ-0014, SGD-41). Silêncio, expiry e observação inconclusiva recusam.
...
3. Fazer uma pergunta atômica; no v5, até três independentes conforme a seção de eficiência.
