# Plan author round 2 — repair P1 only

Revise the five authorized design files under `.agents/skills/speckit-plan/SKILL.md`, keeping the original design except where reviewer P1 requires change. Read the independent `plan-reviewer-result.md` and `continuity-bundle-proof.json` in full. Your report must explicitly say which finding was repaired and cite changed lines. Do not edit code, version files, policy, cache, Store, seals, receipts, or this continuity worktree; do not commit or launch workers.

A detached, clean, preserved Git worktree now exists at `/home/carlosaraujo/orca/workspaces/grill-with-docs/continuity-5544829`, revision `5544829185c2e5d1d75e52736364aa00bede9323`. Its CLI absolute path is `/home/carlosaraujo/orca/workspaces/grill-with-docs/continuity-5544829/plugin/skills/grill-with-docs/scripts/grill_workspace.py`. The repo `continuity-bundle-proof.json` records file SHA-256 and tree ID. From this exact CLI, `gauntlet-orchestration-adopt` read-only preview on the current work item returned PREVIEW with `presentation.work_ready=true`; `validate_block` on current Store passed; pure `prepare_activity` with the current context yielded Codex author `gpt-6-astra/xhigh` and reviewer `gpt-6-astra/high`. No CLI activity prepare was run as a probe, so do not claim that end-to-end test. The installed 6.0.30 cache still fails this Store; no upstream cache edit is allowed.

Make the continuity route concrete: freeze and verify this preserved revision before candidate code changes; use its CLI for v1 campaign coordinator gates that would reject new v1 specialist preparation in candidate; use candidate checkout for implementation and isolated validation only. State precise revalidation against Git revision, tree/hash manifest, Store validation, historical pins, presentation bootstrap and specialist lifecycle through ship. State how to handle a refusal (fail closed; coordinator deterministic fix or GOAL-HOLD), and keep all policy and receipts unchanged. Add scenario combining predecessor without campaign, candidate v1 migration refusal, continued new Codex specialists using preserved bundle. The v1 bundle must never be used to launch new Claude fable specialist; Claude activities under this campaign require a separately justified safe route or a nominal hold. Avoid claiming new Opus on v1 policy where core still requests fable.

Output a durable report outside repo for coordinator copy to `.grill/work-items/feature-latest-models-codex-ab7615f7e67f4e2c9d2b94f507c921ec/plan-author-r2-result.md`; return exact changed file hashes and tests.
