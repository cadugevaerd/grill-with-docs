# Tasks reviewer — independent, read-only

Review `specs/033-latest-models/tasks.md` against `.agents/skills/speckit-tasks/SKILL.md`, the accepted spec/plan, GWD `grill-task-files:v1` template, original source/tests and continuity proof. Inspect all 44 sealed context files as needed; author result is context, not substitute for independent checks. Do not edit, commit, launch workers or advance stages.

Verify 20 sequential IDs, phase/story organization and dependency order, exact Files JSON and Result sidecars, only real writing tasks dispatchable, deferred/read-only order, no concurrent grant overlap, at least one real worker group and realistic partitions. Run the real parser in-memory read-only if possible and report exact verdict. Check all FR/SC coverage: three worker producers, stable retry/remediation, current v2 specialists, v1 historical compatibility with f1475f4, no fable payload, fixture/offline negative effects, eight SemVer points and cumulative CHANGELOG, full validator timing, and continued coordinator use of frozen CLI. Flag any circular prerequisite, source file assigned to two concurrent workers, or leader deferred task mistakenly presented as worker. Do not require a full baseline rerun for this docs-only review.

Return an external durable report for coordinator copy to `.grill/work-items/feature-latest-models-codex-ab7615f7e67f4e2c9d2b94f507c921ec/tasks-reviewer-result.md` with exact verdict `APPROVED` or `CHANGES_REQUIRED`, file/line findings, evidence and residual risk.
...
- `.grill/work-items/feature-latest-models-codex-ab7615f7e67f4e2c9d2b94f507c921ec/tasks-author-observation-closed.json` (sha256 `c193069119bd8f8c81bcb4315ab225d43992f0c21cbc2faf45952d4d323f6ed9`)
- `.grill/work-items/feature-latest-models-codex-ab7615f7e67f4e2c9d2b94f507c921ec/checklist-reviewer-result.md` (sha256 `82a2bc2c0e28215cffe9915023de204dda8bbca796aadccdc80cf8f9cf80fecf`)
