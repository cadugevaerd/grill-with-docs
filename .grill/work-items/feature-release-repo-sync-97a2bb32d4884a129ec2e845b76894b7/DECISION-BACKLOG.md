...
- next-action: nenhuma nesta fase; a melhoria de escopo permanece em SGD-3
- final-ref: ADR-0004
- resolucao: o usuário autorizou explicitamente executar ADR-0004 como decidido — instalar o PAT classic existente como segredo do repositório. O segredo foi gravado via `gh auth token | gh secret set`, por stdin, para que o valor não passasse por linha de comando. A publicação rodou em 2026-08-12, run 31603973983, verde nos três jobs. A decisão registrada em ADR-0004 deixa de ser hipotética e passa a estar em produção, com a consequência que o próprio ADR-0004 já nomeia: qualquer workflow que chegue à main executa com `admin:org`, `admin:enterprise` e `delete_repo`. Reduzir isso continua sendo SGD-3.
- gatilho de retomada: primeira rotação do PAT, qualquer sinal de comprometimento, ou o momento em que um terceiro ganhar permissão de merge no canônico
- ponto de parada: antes de conceder acesso de escrita a qualquer colaborador adicional no repositório canônico
