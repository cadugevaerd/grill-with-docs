...
- impact: high
- state: resolved
- context-refs: contexto de orquestração, checkpoint_head, campaign
...
- impact: high
- state: resolved
- context-refs: rota de recuperação, checkpoint_head, LEADER-TRANSCRIPT-UNPROVEN
...
- impact: high
- state: out-of-scope
- context-refs: checkpoint projetado, autorização humana
...
- impact: medium
- state: out-of-scope
- context-refs: LEADER-TRANSCRIPT-UNPROVEN
- artifacts: docs/adr/, PLAN-CONTEXT.md
- depends-on: DQ-0002
- final-ref: R-0004
...
- impact: high
- state: resolved
- context-refs: quiescência, atividade em voo, TAKEOVER-WORK-ACTIVE
...
- impact: high
- state: resolved
- context-refs: atividade órfã, takeover, autorização humana
...
- depends-on: DQ-0006
- final-ref: R-0006
- resolution: decisão humana (coordenador, msg_ff60609e12bb): entra no escopo. O fence autorizado cobre `DISPATCHED` órfã e `RESULT_RECORDED` com sessão não liberada; mantém não-aceite do resultado, autorização exata work_id+context_id+activity_id, prova terminal Orca e testes negativos. Entregue ao autor como adendo (`agent-orchestration/activities/interview-author-002.payload-addendum.md`) e registrado aqui após o aceite de interview-author-002 para preservar seu `input_sha256`.
- evidence: `interview-author-001` (dispatch ctx_2923e0218c89) ficou `RESULT_RECORDED` com recurso `CLOSE_PENDING`; Orca `releaseState=retained`, `ownershipState=user_owned`, `retainedReason=user_takeover`, liveness `unverifiable`; aceite recusou `SESSION-CLOSE-UNPROVEN` (agent_orchestration.py `accept_activity`; agent_runtime.py:1222-1233).
...
- evidence: proposta de interview-author-002 (result.md, "DQs propostas"); `_require_current_leader` (grill_workspace.py:1658-1671); negativo "líder vivo" decidido em DQ-0006 para a variante órfã.
...
- evidence: `_ACTIVITY_EDGES["RESULT_RECORDED"] == {"RESULT_RECORDED", "ACCEPTED"}` (agent_orchestration.py:48), aplicado em `validate_transition` (1538).
...
- evidence: `activity_id` é chave única (agent_orchestration.py:720); `attempt` write-once (1529) e o CLI grava 1 (grill_workspace.py:5990).
