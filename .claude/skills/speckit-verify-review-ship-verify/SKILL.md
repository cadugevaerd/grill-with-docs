...
report. It is computed by `.specify/extensions/verify-review-ship/scripts/source-fingerprint.sh`, which is the single implementation —
do not restate the algorithm in prose:

```bash
.specify/extensions/verify-review-ship/scripts/source-fingerprint.sh <feature-dir> [config.yml]
```
