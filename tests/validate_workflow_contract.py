...
class Contract(unittest.TestCase):
 ...
 def test_hook_one_real_item_projects_id_branch_step_and_gate(self):
  run('--ensure',str(self.root)); p=subprocess.run([sys.executable,str(WS),'init',str(self.root),'--runtime','claude','--type','feature','--slug','alpha','--work-id','wx','--skip-backlog'],text=True,capture_output=True); self.assertEqual(p.returncode,0,p.stdout+p.stderr); before=snapshot(self.root)
 ...
 def test_hook_many_real_items_lists_ids_and_work_id_guidance(self):
  run('--ensure',str(self.root))
  for wid in ('wx','wy'): self.assertEqual(subprocess.run([sys.executable,str(WS),'init',str(self.root),'--runtime','claude','--type','feature','--slug',wid,'--work-id',wid,'--skip-backlog'],capture_output=True).returncode,0)
