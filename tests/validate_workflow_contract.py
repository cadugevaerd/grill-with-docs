...
HERE=Path(__file__).resolve(); REPO=HERE.parents[1]; PLUGIN=REPO/'plugin'; SCRIPT=PLUGIN/'skills/grill-with-docs/scripts/ensure_workflow.py'; WS=PLUGIN/'skills/grill-with-docs/scripts/grill_workspace.py'; ASSETS=PLUGIN/'skills/grill-with-docs/assets'; TEMPLATE=ASSETS/'WORKFLOW.template.md'; TEMPLATE_V4=ASSETS/'WORKFLOW.v4.template.md'; REGISTRY_V4=ASSETS/'workflow-step-skills.v4.json'; HOOKS=PLUGIN/'hooks/hooks.json'; MARK='grill-with-docs-workflow:v2'; MARK_V4='grill-with-docs-workflow:v4'
...
class Contract(unittest.TestCase):
 ...
 def test_hook_one_real_item_projects_id_branch_step_and_gate(self):
  run('--ensure',str(self.root)); p=subprocess.run(orchestration_fixture.command(WS, ('init',str(self.root),'--runtime','claude','--type','feature','--slug','alpha','--work-id','wx','--skip-backlog')),text=True,capture_output=True); self.assertEqual(p.returncode,0,p.stdout+p.stderr); before=snapshot(self.root)
 ...
 def test_hook_many_real_items_lists_ids_and_work_id_guidance(self):
  run('--ensure',str(self.root))
  for wid in ('wx','wy'): self.assertEqual(subprocess.run(orchestration_fixture.command(WS, ('init',str(self.root),'--runtime','claude','--type','feature','--slug',wid,'--work-id',wid,'--skip-backlog')),capture_output=True).returncode,0)
 ...
 def test_hook_unsafe_status_is_blocked_without_external_read(self):
  run('--ensure',str(self.root)); self.assertEqual(subprocess.run(orchestration_fixture.command(WS, ('init',str(self.root),'--runtime','claude','--type','feature','--slug','alpha','--work-id','wx','--skip-backlog')),capture_output=True).returncode,0); state=self.root/'.grill/work-items/wx/state.json'; external=Path(self.t.name)/'secret'; external.write_text('TOP-SECRET'); state.unlink(); state.symlink_to(external); before=external.read_bytes()
