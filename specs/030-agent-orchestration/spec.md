...
### User Story 8 - Usar o estilo padrão do GWD nos dois ambientes (Priority: P1)

Como operador, quero i-have-adhd obrigatório na stack e aplicado automaticamente pela skill GWD atualizada em Codex e Claude Code, com funcionamento comprovado e sem alterar o padrão das sessões fora do GWD.

**Why this priority**: Instalar o componente não garante que suas regras sejam carregadas nem que as respostas as sigam; o padrão precisa acompanhar o uso do GWD nos dois ambientes.

**Independent Test**: Iniciar sessões novas de GWD em cada ambiente, sem invocar i-have-adhd manualmente, verificar carregamento e respostas representativas e comparar com sessões externas ao GWD.

**Acceptance Scenarios**:

1. **Given** a skill GWD atualizada e seus pré-requisitos disponíveis, **When** o operador inicia ou retoma o GWD em cada CLI, **Then** i-have-adhd está instalado e habilitado e seu estilo é aplicado por padrão, sem invocação manual do componente.
2. **Given** sessão nova de cada ambiente no GWD, **When** o operador solicita orientação com várias ações e continua a conversa, **Then** as respostas observadas seguem as regras de apresentação do i-have-adhd e a evidência distingue versão, habilitação, carregamento automático e comportamento, sem reduzir o conteúdo exigido pelo trabalho.
3. **Given** componente ausente, desabilitado, incompatível, carregamento não confirmado ou inicialização impedida por confiança ainda não concedida, **When** o GWD verifica a stack, **Then** apresenta o pré-requisito específico que falta e não declara o modo funcional nem o substitui silenciosamente.
4. **Given** configurações existentes e política Ponytail, **When** a integração do GWD é aplicada ou atualizada, **Then** conserva as configurações alheias e a política de implementação, sem ativar o estilo globalmente.
5. **Given** sessão fora do projeto/fluxo GWD, **When** o operador usa qualquer dos dois CLIs após a atualização, **Then** o padrão anterior dessa sessão é preservado.
6. **Given** instalação confirmada, teste isolado bem-sucedido ou saída de sucesso sem evidência de carregamento e resposta real, **When** a entrega é avaliada, **Then** essas evidências isoladas não satisfazem o aceite de funcionamento.
...
- Aprovar um hook permite sua execução, mas não comprova ativação do estilo no GWD; inicialização bem-sucedida também não comprova comportamento.
- Um estilo de apresentação existente não pode mascarar a ausência de carregamento do i-have-adhd; a prova precisa identificar o componente e o alcance do padrão.
- Respostas concisas continuam preservando conteúdo, decisões, evidências e exigências explícitas; a apresentação não reduz o escopo da execução.
...
- **FR-020**: A entrega MUST abranger plugin, core CLI, contratos, integração das skills, documentação e verificações dos oito requisitos, com rastreabilidade ao work item e gates constitucionais de versão e publicação. Registros MUST ser descritos como evidência estrutural auditável, sem alegar prova criptográfica de execução de modelo ou skill. Aceite: auditoria de cobertura dos oito requisitos, distribuição e evidências de verify/review/ship.

- **FR-021**: i-have-adhd MUST integrar a stack obrigatória instalada e habilitada em Codex e Claude Code; a skill GWD atualizada MUST aplicar seu estilo por padrão ao iniciar e retomar o GWD, sem invocação manual do componente. Aceite: US8.1.
- **FR-022**: O padrão MUST limitar-se ao projeto/fluxo GWD e MUST preservar o padrão das sessões externas, as configurações alheias e a política de implementação Ponytail. Aceite: US8.4–5.
- **FR-023**: A verificação MUST distinguir presença/versão, habilitação, carregamento automático e comportamento observado; ausência, incompatibilidade, confiança pendente ou evidência insuficiente MUST produzir diagnóstico específico sem alegação de funcionamento ou substituição silenciosa. Aceite: US8.3, US8.6.
- **FR-024**: O aceite MUST incluir evidência de carregamento automático e respostas reais em sessões novas dos dois ambientes, preservando integralmente o conteúdo exigido e comprovando o alcance local; instalação, configuração ou teste isolado MUST NOT substituir essa prova. Aceite: US8.2, US8.4–6.
...
- **Estado do estilo GWD**: Componente e versão, ambiente, alcance do padrão, habilitação, evidência de carregamento e amostra comportamental ou diagnóstico do impedimento.
...
- **SC-007**: A auditoria da entrega encontra cobertura verificável dos oito requisitos em comportamento, documentação e verificações, com gates constitucionais satisfeitos antes da publicação.

- **SC-008**: As quatro combinações de início/retomada e ambiente aplicam o estilo sem invocação manual; cada ambiente possui evidência de sessão nova com carregamento e respostas conformes, zero perda do conteúdo exigido e zero alteração do padrão de sessões externas ao GWD. Todos os cenários de pré-requisito ausente ou prova insuficiente são diagnosticados sem falso sucesso.

## Assumptions

- O handoff aprovado é a fonte de escopo. Os oito requisitos pertencem à mesma fase e entrega; prioridades das histórias não autorizam omitir requisitos.
...
- i-have-adhd é uma restrição de produto explicitamente aprovada no requisito 8, não escolha de biblioteca interna. Sua instalação 0.3.0 observada nesta máquina e a aprovação do hook não comprovam a integração futura. Plan definirá ativação local e verificação, mantendo a instalação delegada e o core sem dependências externas.
