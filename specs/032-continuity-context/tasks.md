...
- [X] T054 Em `plugin/skills/grill-with-docs/scripts/grill_workspace.py`, no bloco de comentário que introduz a tupla estrutural: estreitar a afirmação de que a branca é carimbada mas nunca comparada, que é falsa desde que o ponto único passou a comparar a branca viva contra o vínculo do work item seis linhas abaixo, e acrescentar uma linha apontando que a comparação existe, contra outra fonte e por outra função; não acrescentar parágrafo explicativo nem mover a explicação, porque ela já existe correta em dois outros pontos e um terceiro exemplar recria o achado de regra duplicada; corrigir na mesma passagem o comentário do sítio de cunhagem da confirmação de etapa, que afirma que a branca viva já é a árvore em que o contexto rodou, quando a validação a montante é de projeto, caminho real e diretório comum, nunca de branca, e duas brancas na mesma worktree passam idênticas per FR-010 (contradicts)
- [X] T055 Em `plugin/skills/grill-with-docs/scripts/grill_workspace.py`, na derivação de identidade de continuidade: validar o tipo do bloco de desenvolvimento antes de lê-lo, ou resolver a fase sem assumir mapa, de modo que bloco presente e de tipo errado produza a recusa nomeada por esquema em vez de falha genérica; hoje a derivação roda nos três verbos antes da guarda que nomeia essa recusa, e só o curto-circuito de uma disjunção a salva quando a fase ativa é verdadeira, de modo que a recusa é inalcançável exatamente quando a fase ativa é nula — estado obrigatório em milestone terminal e presente em metade dos work items reais deste repositório per FR-010 (partial)
- [X] T056 Em `tests/validate_agent_orchestration_contract.py`, no caso que exercita a cunhagem do vínculo: remover o subcaso de carimbo coincidente, que depois da remoção da comparação é comportamentalmente idêntico ao subcaso sem carimbo e cuja leitura de volta do valor selado não detecta a regressão que o comentário dele afirma detectar, conforme provado por mutação — suprimir a escrita da branca no enxerto de sucessão derruba apenas o subcaso contraditório; remover junto o comentário que faz essa afirmação falsa, e corrigir a asserção de desigualdade que roda também quando o vínculo é nulo, onde é trivialmente verdadeira per FR-011 (partial)
- [X] T057 Em `tests/validate_agent_orchestration_contract.py`, no caso que cobre a recusa por esquema do bloco de desenvolvimento: remover a semeadura de fase ativa que hoje é necessária para que a guarda sob teste seja alcançada, depois que a derivação de identidade passar a validar o tipo, e afirmar a recusa com fase ativa nula, que é a configuração em que ela hoje não acontece; o caso atual documenta a própria muleta em comentário, o que é honesto, mas prova a guarda apenas no caminho em que ela já funcionava per FR-011 (partial)
...
- [X] T058 Em `plugin/skills/grill-with-docs/scripts/grill_workspace.py`, no comentário do sítio de cunhagem da virada de fase: substituir a alegação de que os verbos de continuidade provam a branca viva a montante pelo texto já corrigido no sítio irmão da confirmação de etapa, que diz o contrário e é o verdadeiro — a branca saiu da tupla estrutural, os guardas estruturais não provam nada sobre ela, e nenhum deles roda neste caminho; antes de dar a tarefa por concluída, varrer o repositório inteiro pela alegação e por suas variantes e confirmar que nenhuma outra cópia sobreviveu, porque a fase anterior corrigiu uma de três cópias e a alegação remanescente é a mesma que originou o achado crítico da sétima rodada per FR-010 (contradicts)
- [X] T059 Em `plugin/skills/grill-with-docs/scripts/grill_workspace.py`, na documentação do ponto único de comparação de vínculo: deixar de anunciar a guarda de esquema que ele aplica como se fosse a guarda efetiva, e declarar que hoje ela é defesa em profundidade inalcançável pelos três verbos, porque a derivação de identidade recusa o mesmo conjunto de entradas antes; manter a guarda, e não removê-la, porque a assinatura aceita valor de qualquer tipo, o projeto é fail-closed e removê-la deixaria armadilha para um quarto chamador que não passe pela derivação per FR-010 (partial)
- [X] T060 Em `tests/validate_agent_orchestration_contract.py`, no caso que cobre a recusa por esquema pela retomada: repontar a documentação para nomear a guarda que ele de fato exercita hoje, que é a da derivação de identidade, e substituir o recibo de reversão, que afirma um resultado que deixou de ocorrer — reverter a guarda nomeada já não faz o caso seguir adiante, porque a recusa passou a vir de outro ponto; o recibo novo deve registrar o resultado medido da reversão correta, que é falha genérica por erro de atributo per FR-011 (contradicts)
- [X] T061 Em `tests/validate_agent_orchestration_contract.py`: acrescentar cobertura própria à guarda de esquema do ponto único de comparação de vínculo, chamando-a diretamente com bloco de desenvolvimento de tipo errado e afirmando a recusa nomeada, já que nenhum caminho de linha de comando a alcança e hoje trocá-la por continuação silenciosa deixa a suíte inteira verde; e corrigir a documentação do caso de cunhagem, que anuncia três situações de carimbo quando a do meio foi removida de propósito, mais o ramo condicional que seleciona essa situação e ficou sem seletor per FR-011 (missing)
- [X] T062 Em `tests/validate_agent_orchestration_contract.py`: cobrir a cláusula que preserva bloco de desenvolvimento ausente como estado legítimo, hoje sem teste algum — transformá-la em recusa deixa a suíte verde, e ausência é estado real, não hipótese, de modo que uma regressão ali recusaria work items legítimos sem aviso; o caso deve remover o bloco, usar fase ativa nula e afirmar que o verbo segue, com o digest do armazenamento inalterado per FR-011 (missing)
- [X] T063 Em `tests/validate_agent_orchestration_contract.py`, nas duas asserções de desigualdade do caso de vínculo e do caso de cunhagem: remover as que comparam um literal fixo com a branca da fixture, porque são tautológicas e neutralizá-las não reprova nada, ou mantê-las declarando no próprio caso que são sanidade de fixture e não cobertura; o registro do nó anterior afirma que movê-las as tornou não-triviais, o que não é verdade, e esse registro também precisa ser corrigido per FR-011 (partial)
...
- [X] T064 Em `plugin/skills/grill-with-docs/scripts/grill_workspace.py`, no atalho que devolve reuso na adoção de orquestração: repor a igualdade de origem na condição, alinhando-a à precedência que o caminho de aplicação usa; a checagem foi retirada dali quando o ponto de conflito passou a recusar toda origem alterada, e a integração com a linha principal afrouxou exatamente esse ponto, de modo que a implicação que sustentava a retirada deixou de valer — hoje origem alterada com escopo, apresentação e líder iguais devolve reuso, veredito que afirma repetição idêntica sobre entrada que não é idêntica; o estado em disco não diverge, e por isso nenhum teste reprova per FR-010 (contradicts)
- [X] T065 Em `CHANGELOG.md`: restaurar a seção da versão publicada imediatamente anterior, que a resolução do segundo merge renomeou em vez de preservar, com o item que de fato saiu naquela versão, e abrir a seção desta entrega acima dela contendo apenas os itens desta entrega; a versão renomeada tem tag e release publicados, e a cláusula constitucional de release por versão a torna registro público, de modo que perder a seção faz um item já entregue reaparecer como novidade; remover também a linha em branco solta deixada no meio da lista per cláusula `Release obrigatória por versão` (contradicts)
- [X] T066 Em `tests/validate_agent_orchestration_contract.py`: cobrir a paridade entre prévia e aplicação da adoção de orquestração no caminho de origem alterada, exercitando os dois na mesma entrada e exigindo o mesmo veredito; hoje o único caso de paridade cobre a recusa por cerca de líder, e o invariante que esta entrega criou está sustentado por alinhamento manual entre duas funções, não por prova — foi essa ausência que permitiu à integração invertê-lo sem aviso, e a suíte só reprovou porque um caso vindo da linha principal exercitava o caminho per FR-011 (missing)
- [X] T067 Em `tests/validate_agent_orchestration_contract.py`: cobrir a chamada do ponto único de comparação de vínculo na preparação de troca e na tomada de contexto, hoje sem prova alguma — trocar qualquer uma das duas por continuação deixa a suíte inteira verde; são exatamente as duas metades que a sexta rodada de review mandou acrescentar, com o argumento de que a preparação cria a operação, grava o ponto de retomada e libera o condutor antes de qualquer checagem, de modo que a divergência só apareceria na retomada com o contexto já solto; cada caso deve semear o vínculo numa branca diferente da viva e exigir a recusa nomeada com o digest do armazenamento inalterado per FR-011 (missing)
- [X] T068 Em `tests/validate_agent_orchestration_contract.py`, no caso direto que já exercita o ponto único: acrescentar a afirmação de que o comparador de identidade recusa carimbo presente porém de tipo errado, hoje sem prova — inverter o retorno desse ramo deixa a suíte verde e faz carimbo não-mapeamento ser aceito nos três comparadores; é guarda de fronteira e fail-closed, categoria que o projeto não simplifica por economia per FR-011 (missing)
