...
**Observação da instalação do `i-have-adhd` por runtime** (desde a 6.0.2): quando a listagem nativa traz `installPath` absoluto, ele é usado direto, em qualquer runtime — é o caso do Claude. No Codex, `codex plugin list --json` (codex-cli 0.154.0) não emite caminho algum, então o caminho é **composto** de `marketplaceName`, `name` e `version` da própria entrada nativa, sob `CODEX_HOME` (ou `~/.codex`) em `plugins/cache/`, e só é aceito com `installed=true`, segmentos que sejam nomes simples (não vazios, diferentes de `.` e `..`, sem barra, contrabarra ou drive do Windows) e o `SKILL.md` presente em disco. O conteúdo continua julgado depois por `approved_presentation_reference`, comum aos dois runtimes; falha de acesso ou home ausente mantém a instalação indeterminada, nunca vira exceção.
...
- **Grant do `partition` é léxico: todo token com `/` numa linha de tarefa vira caminho de grant**: `NEEDS-BIND`/`NEEDS-CREATE`, `name`/`code`, `<tmp>/linked` ou `refs/heads/x` dentro da descrição entram em `files` do nó e o worker recebe grant de um arquivo que não existe. Aconteceu na 028 e na 029 (`analysis.md` P1 de ambas). Antes de `partition`, rode `partition-emit` em preview e leia `execution_dag.nodes[].files`; reescreva a linha sem a barra em vez de editar o DAG.
