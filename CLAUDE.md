...
**Observação da instalação do `i-have-adhd` por runtime** (desde a 6.0.2): no Claude a listagem nativa traz `installPath` e ele é usado direto. No Codex, `codex plugin list --json` (codex-cli 0.154.0) não emite caminho algum, então o caminho é **composto** de `marketplaceName`, `name` e `version` da própria entrada nativa, sob `CODEX_HOME` (ou `~/.codex`) em `plugins/cache/`, e só é aceito com `installed=true`, segmentos que sejam nomes simples (sem barra, contrabarra ou drive do Windows) e o `SKILL.md` presente em disco. O conteúdo continua julgado depois por `approved_presentation_reference`, comum aos dois runtimes; falha de acesso ou home ausente mantém a instalação indeterminada, nunca vira exceção.
